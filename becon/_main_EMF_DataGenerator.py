...
def generateFirstOrderDifferenceWords(ticker, periods=[]):
	...
	dg_handle.feedDataSeries(ticker)
	...
	for periodLength in periods:
		...
		dg_handle.feedWordSeries(wordTicker, wordSubType, wordType, wordSuperType)
		...
		EM_DGUtil.insertWords(dates, values)
		EM_DGUtil.resetWordSeries()
	...
	for index, ticker in enumerate(tickers):
		...
		while i < len(dataSeries):
			...
			for j in count:
				...
				value = j
				(success, error) = EM_DBHelp.insertDataPoint_WordHistoryTable(	db_conn, db_curs, 
																				wordSeriesID, 
																				date, 
																				value)
				if success:
					...
					log.warning('\t\tFailed to Write Historical Word %s for %s [value = %f]', wordTicker, date, value)
		...
		columnName = 'int_unsuccessful_generations'; value = unsuccessfulInserts; EM_DGUtil.sendToDB() # Update Successful Inserts
		columnName = 'int_word_periodicity'; value = dataPeriodicity; EM_DGUtil.sendToDB() # Update Word Periodicity
		columnName = 'dt_earliest_word'; value = minDate; EM_DGUtil.sendToDB() # Update Earliest Word
		columnName = 'dt_latest_word'; value = maxDate; EM_DGUtil.sendToDB() # Update Latest Word
		columnName = 'dt_last_generated'; value = EM_util.dtGetNowAsEpoch(); EM_DGUtil.sendToDB() # Update Last Update Date
